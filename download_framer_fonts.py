...
def extract_framer_font_urls(html_content):
    """从HTML内容中提取所有framerusercontent.com字体URL"""
    # 匹配framerusercontent.com的字体URL，更精确的模式
    pattern = r'https://framerusercontent\.com/assets/[a-zA-Z0-9_-]+\.woff2'
...
def main():
    ...
    # 提取字体URL
    font_urls = extract_framer_font_urls(html_content)
    print(f"找到 {len(font_urls)} 个唯一framer字体URL")
    ...
    print(f"\nframer字体下载完成! 成功: {success_count}/{len(font_urls)}")
...
def generate_font_mapping(font_urls, static_dir):
    ...
    # 保存映射到文件
    with open('framer_font_mapping.txt', 'w', encoding='utf-8') as f:
        for original_url, local_path in mapping.items():
            f.write(f"{original_url} -> {local_path}\n")
    
    print(f"framer字体映射已保存到 framer_font_mapping.txt")
